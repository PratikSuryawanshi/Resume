...
void main(int argc, char *argv[])
{
  int count;
  long **K, **L;
  ushort i, j, a, b, c, d, key[8] = {1, 2, 3, 4, 5, 6, 7, 8};
  ushort X[4], Y[4];
  ...
  count = fread( Y,2,4,mid_file);

  while(count==4){

    IDEA_encryption(Y, X, L);
    fwrite( X,2,4,out_file);
    count = fread( Y,2,4,mid_file);
  }
	if(count < 4 && count > 0){
